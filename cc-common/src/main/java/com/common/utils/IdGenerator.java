...
IdGenerator {
    ...
    public static synchronized IdGenerator getInstance(long pid) {
        if (idGenerator == null) {
            idGenerator = new IdGenerator(pid);
        }
        return idGenerator;
    }
    ...
    public IdGenerator(long processId) {
        if (processId > ((1 << processIdBits) - 1) || 0 == processIdBits) {
            System.out.println("进程ID超出范围，设置位数" + processIdBits + "，最大" + ((1 << processIdBits) - 1) + "强制为1023");
            this.processId = 1023;
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < 100; i++) {
            System.out.println("bbb" + (IdGenerator.getInstance(Thread.currentThread().getId()).getId()));
        }
        new Thread() {
            public void run() {
                for (int i = 0; i < 100; i++) {
                    System.out.println("aaa" + (IdGenerator.getInstance(Thread.currentThread().getId()).getId()));
                    // System.out.println();
